...
express()
  ...
  .set('view engine', 'ejs')
  .get('/s1', (req,res) => res.render('pages/s1'))
  ...
  .get('/', async (req, res) => {
    try{
      ...
      const result2 = await client.query('select * from lastuser');
      //const results2 = {'results': (result2) ? result2.rows : null};
      res.render('pages/index', {results: result.rows, results2: result2.rows});
      client.release();
    } catch(err) {
      console.error(err);
      res.send("Error "+err);
